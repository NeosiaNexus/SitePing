...
|---|---|
| Prisma schema | Both models present, every field up to date — the exact diff `sync` applies, so type, optionality, attributes (`@unique`, `@db.Text`, …) and `@@index` blocks all count |
...
`doctor` sends one GET request to your running server and tells you whether a SitePing handler answered (10-second timeout). Pass **both** `--url` and `--endpoint` to run it non-interactively — either one you omit becomes a prompt. When the endpoint is configured with an `apiKey` and `GET` is not public, pass the key with `--api-key`; without it, a `401` comes with a hint.
